#!/usr/bin/env python3
"""
Train the ERP<->Gateway Residual XGBoost Classifier.

Optimizes for PR-AUC on hard negatives and exports model and calibrated
probability threshold artifacts.

Adapted from train_model.py (GW<->Bank). Key differences:
- Data: data/raw/erp_gw_train_features.csv
- Model artifact: src/ai/artifacts/xgb_erp_gw.json
- Threshold artifact: src/ai/artifacts/xgb_erp_gw_threshold.json
- Feature columns: from erp_gw_features.FEATURE_COLUMNS
"""
...
from src.ai.erp_gw_features import FEATURE_COLUMNS
...
def train_and_export():
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            f"ERP<->GW training dataset not found at {DATA_PATH}. "
            "Run erp_gw_dataset_builder.py first."
        )
    ...
    print("[*] Training ERP<->GW XGBoost classifier...")
